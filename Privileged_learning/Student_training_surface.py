...
from model2 import Student, mdn_loss,mdn_sample
from sb3_contrib import TQC

import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--student-model-dir', default='./model/surface/', type=str, help='save student')
parser.add_argument('--expert-model-dir', default='../../model_checkpoints/surface/model', type=str, help='load expert')
...
student_net = Student(input_size = input_dim, action_space=action_dim,hidden_size=256, device=device).to(device)
...
def train(data_all,student_net,train_epochs):
    ...
    for epoch in range(train_epochs):
        ...
        for i, data in enumerate(train_data_loader):
            ...
            mu, sigma, pi = student_net(obs)
            loss = mdn_loss(pi, sigma, mu, action)
            optimizer.zero_grad()
            loss.backward() 
            optimizer.step()  
...
def collect(env,student_net, data_set,collet_epoch,count):
    ...
    for i in tqdm(range(collet_epoch)):
        ...
        for i in range(200):
            ...
            with torch.no_grad():
                mu, sigma, pi = student_net(obs_batch)
                student_action = mdn_sample(mu, sigma, pi).cpu().numpy()[0]
...
if __name__ == "__main__":
    ...
    if args.load_data:
        if os.path.exists(model_dir + 'data.pkl'):        
            ...
            count,data_all = collect(env,student_net,data_all,collet_epoch=args.collect_epochs * 5, count=0)
            with open('./model/surface/data.pkl', 'wb') as file:
